...
引数にfreqWave追加
外部から周波数指定するようにした。
max fevを十分大きな数に変更することでfittingできない問題を解決
<PLAN>
'''
...
def fitting(rawdata_directory,filebasename,freqWave):
	...
	## __INDICATE CONDITION__________________________
		...
		## __FITTING LOG__________________________
			fittinglog="paramater%s = %s" % (str(freqFit), paramater_optimal)
			print(fittinglog)   #fitting結果の表示
...
print(fitting(rawdata_directory,filebasename,freqWave))
