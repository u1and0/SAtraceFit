...
def fitting(rawdata_directory,filebasename,freqWave):
	...
	## __FITTING RANGE__________________________
		...
		print('\tSNratio=',SNratio,'fittingFreqFit=',fittingFreqFit,'waveWidth=',waveWidth)

	## __INDICATE CONDITION__________________________
		# if eval(indicateCondition) :   #indicateConditionにマッチしたウェーブだけをプロットする
		def function(waveWidth,SNratio,freqFit,fittingFreqFit,datay,yy,datax,fity):
			if waveWidth<=15 and SNratio>10 and abs(freqFit-fittingFreqFit)<0.1 :
				print('This is Carrier Wave.')
				SNratio=datay[freq2pnt(freqFit)]-yy
				plt.plot(pnt2freq(datax),fity,'-',lw=2,label=str(freqFit)+"kHz")   #fitting結果のプロット
			elif 15<waveWidth<50 and abs(freqFit-fittingFreqFit)<0.1:
				print('This is Broad Wave.')
				plt.plot(pnt2freq(datax),fity,'-',lw=2,label=str(freqFit)+"kHz")   #fitting結果のプロット
			else :
				print('This is not Wave.')
			return SNratio

		## __FITTING LOG__________________________
			# print('\tOK! Plot as fit data...')
		## __OUTPUT__________________________
			fittingDict[str(freqFit)+'kHz']=function(waveWidth,SNratio,freqFit,fittingFreqFit,datay,yy)  #周波数をキー、SN比を値にしてfittngDictへ入れる
